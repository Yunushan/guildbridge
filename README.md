...
Apply-side content imports can write journals, reports, lock files, incremental state, and dead-letter files. A content resume journal is bound to the provider, target, and exact ordered action fingerprint; changing the archive, channel map, target, or generated plan requires a new reviewed run:
...
Confirmed apply runs require a reviewed dry-run plan through `--plan-in`. GuildBridge recomputes a no-write candidate plan, compares the command, target, template fingerprint, action count, and action hash to the reviewed file, and refuses writes if anything drifted. Apply runs also write a local journal and acquire a target-scoped structural lock before provider writes start. By default the journal is saved under `.guildbridge/journals/` and the lock under `.guildbridge/locks/`; use `--journal-out path/to/journal.json` or `--lock-file path/to/lock` for explicit paths. If a run fails halfway through, inspect the journal before retrying and pass `--resume-journal path/to/journal.json` so GuildBridge verifies the retry uses the same command, target, provider, template fingerprint, and reviewed plan hash. A pre-existing lock fails closed; remove it only after confirming no migration process is still running.

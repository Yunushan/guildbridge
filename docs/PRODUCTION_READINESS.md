...
- Tag releases verify the tag version, generate wheel/sdist and Windows ZIP/MSI outputs, publish SHA-256 checksum files and an SPDX 2.3 SBOM, create GitHub build provenance attestations, cryptographically verify those attestations against the exact repository, release workflow, tag, commit, and GitHub-hosted runner, and compare every downloaded release asset to both its manifest and the protected private evidence record before publication.
...
4. A clean consumer machine verifies the published `SHA256SUMS` and reviews the release SPDX SBOM before installing the wheel, ZIP, or MSI.
